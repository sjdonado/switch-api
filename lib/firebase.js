  ...
  return tmpFile(file.buffer)
    .then(async (path) => {
      ...
      return storage.file(`profile_pictures/${fileName}`).getSignedUrl({
        ...
        .then(signedUrls => ({ url: signedUrls[0], ref: `profile_pictures/${fileName}` }))
        .catch(err => next(err));
    })
    .catch(err => next(err));
};
...
module.exports.getUser = async (user) => {
  try {
    const userInfo = await db.collection('users')
      .where('uid', '==', user.uid)
      .get();
    const data = Object.assign({ id: userInfo.docs[0].id }, userInfo.docs[0].data());
    return userInfo.size > 0 ? data : user;
  } catch (e) {
    return e;
  }
};
